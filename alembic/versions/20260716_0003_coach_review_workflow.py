...
review_visibility = postgresql.ENUM(
    "coach_only",
    "athlete_visible",
...
review_rejection_category = postgresql.ENUM(
    "inaccurate",
    "insufficient",
    "unsafe",
    "irrelevant",
...
review_audit_action = postgresql.ENUM(
    "review_generated",
    "revision_created",
    "preview_requested",
    "review_approved",
...
def upgrade() -> None:
    ...
    op.execute(
        """
        UPDATE ai_reviews
        SET latest_revision_number = COALESCE(
            (
                SELECT MAX(revision_number)
                FROM review_revisions
                WHERE review_revisions.review_id = ai_reviews.id
            ),
            0
        )
        """
    ...
    op.execute(
        """
        UPDATE ai_reviews
        SET generated_at = generation_completed_at
        WHERE status IN ('generated','approved','rejected')
        """
    )
    ...
    op.add_column(
        "review_revisions",
        sa.Column("based_on_revision_number", sa.Integer(), nullable=True),
    )

    op.create_check_constraint(
        "ck_revision_number_positive",
        "review_revisions",
        "revision_number > 0",
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "source_revision_id",
            sa.UUID(),
            sa.ForeignKey("review_revisions.id"),
        ),
        sa.Column(
            "approved_by_user_id",
            sa.UUID(),
        ...
        sa.Column("athlete_message", sa.Text()),
        sa.Column(
            "visibility",
            review_visibility,
            nullable=False,
...
def downgrade() -> None:
    ...
    review_audit_action.drop(op.get_bind(), checkfirst=True)
    review_rejection_category.drop(op.get_bind(), checkfirst=True)
    review_visibility.drop(op.get_bind(), checkfirst=True)